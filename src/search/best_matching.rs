...
/// Get the best attributes for the emoji based on its keywords matching against
/// the input words
...
/// Calculate the number of different types of matches between input words and
/// keywords
fn get_num_matches(
	input_words_array: &[String],
	stemmed_input_words_array: &[String],
...
fn compare_attributes(a: &Attributes, b: &Attributes) -> Ordering {
	...
	if a.num_prefix_stemmed_word_matches != b.num_prefix_stemmed_word_matches {
		return b.num_prefix_stemmed_word_matches.cmp(&a.num_prefix_stemmed_word_matches);
	}
